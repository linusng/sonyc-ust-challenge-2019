...
def load_embeddings(file_list, emb_dir):
    ...
    for idx, filename in enumerate(file_list):
        ...
        with open(emb_path, 'rb') as f:
            npz = np.load(f)
            embeddings.append(npz.f.embedding)
...
def train_model(model, X_train, y_train, X_valid, y_valid, output_dir,
    ...
    # FOR TENSORBOARD - added Monday 13th May 2019
    cb.append(keras.callbacks.TensorBoard(log_dir='./Graph',
                                          histogram_freq=0,
                                          write_graph=True,
                                          write_images=True))
...
def train_framewise(annotation_path, taxonomy_path, emb_dir, output_dir, exp_id,
    ...
    #embeddings = embeddings.reshape((2732,64,768))
    print("embs shape = ", embeddings.shape)
    ...
    #model = linus_rnn(emb_size, num_classes)
